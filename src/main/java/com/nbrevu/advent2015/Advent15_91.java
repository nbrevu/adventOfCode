...
public class Advent15_91 {
	private final static String IN_FILE="2015/Advent91.txt";
	...
	private static class Graph<T>	{
		...
		private static class Accumulator	{
			...
			public Accumulator()	{
				minimum=OptionalInt.empty();
			}
			public void accumulate(int newCycle)	{
				if (minimum.isEmpty()||(newCycle<minimum.getAsInt())) minimum=OptionalInt.of(newCycle);
			}
		...
		public OptionalInt minDistance()	{
			Accumulator distanceAccumulator=new Accumulator();
			Set<T> visited=new HashSet<>();
			for (T key:edges.keySet())	{
				visited.add(key);
				minDistanceRecursive(key,visited,distanceAccumulator,0);
				visited.remove(key);
			}
			return distanceAccumulator.getMinimum();
		}
		private void minDistanceRecursive(T current,Set<T> visited,Accumulator distanceAccumulator,int totalDistance)	{
			...
			for (Node<T> edge:edges.get(current)) if (!visited.contains(edge.destination))	{
				...
				minDistanceRecursive(edge.destination,visited,distanceAccumulator,totalDistance+edge.distance);
	...
	public static void main(String[] args) throws IOException	{
		...
		OptionalInt result=graph.minDistance();
