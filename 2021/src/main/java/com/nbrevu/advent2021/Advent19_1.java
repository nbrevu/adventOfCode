...
public class Advent19_1 {
	...
	private static class Transformation	{
		...
		public Point reverseTransform(Point p)	{
			int x=shiftX+Rotation.reassignPointDirection(p,rotation.xAssignedTo);
			int y=shiftY+Rotation.reassignPointDirection(p,rotation.yAssignedTo);
			int z=shiftZ+Rotation.reassignPointDirection(p,rotation.zAssignedTo);
			return new Point(x,y,z);
		}
	}
	...
	private static Set<Point> getUnifiedPoints(List<Scanner> scanners)	{
		Transformation[] transforms=getTransformations(scanners);
		Set<Point> result=new HashSet<>();
		for (int i=0;i<scanners.size();++i)	{
			Transformation t=transforms[i];
			for (Point p:scanners.get(i).relativePoints.keySet()) result.add(t.reverseTransform(p));
		}
		return result;
	}
	...
	public static void main(String[] args) throws IOException	{
		...
		System.out.println(getUnifiedPoints(scanners).size());
