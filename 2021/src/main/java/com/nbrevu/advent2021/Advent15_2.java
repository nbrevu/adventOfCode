...
public class Advent15_2 {
	...
	private static int[][] expand(int[][] original,int multiplier)	{
		int h=original.length;
		int w=original[0].length;
		int[][] result=new int[5*h][5*w];
		for (int i=0;i<result.length;++i) for (int j=0;j<result[i].length;++j)	{
			int baseI=i%h;
			int qI=i/h;
			int baseJ=j%w;
			int qJ=j/w;
			int number=original[baseI][baseJ]+qI+qJ;
	...
	public static void main(String[] args) throws IOException	{
		...
		System.out.println(new OptimalPaths(expand(numbers,5)).getBestResult());
