...
public class Advent12_1 {
	...
	private static class Graph	{
		...
		public class GraphPath	{
			...
			private GraphPath(Node currentNode,Set<Node> alreadyVisited)	{
			...
			public List<GraphPath> traverse()	{
				...
				for (Node n:getNextNodes(currentNode)) if (n.isBig()) result.add(new GraphPath(n,alreadyVisited));
				else if (!alreadyVisited.contains(n))	{
					Set<Node> newAlreadyVisited=new HashSet<>(alreadyVisited);
					newAlreadyVisited.add(n);
					result.add(new GraphPath(n,newAlreadyVisited));
				}
				return result;
			}
		}
		public GraphPath getInitialPath()	{
			...
			return new GraphPath(start,Set.of(start));
		}
	}
