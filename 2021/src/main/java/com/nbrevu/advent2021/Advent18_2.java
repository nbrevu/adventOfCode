...
public class Advent18_2 {
	...
	private static class SnailPair extends SnailNumber	{
		...
		@Override
		public SnailPair copy()	{
			return new SnailPair(left.copy(),right.copy());
		}
	}
	...
	private static class SnailLiteral extends SnailNumber	{
		...
		@Override
		public SnailLiteral copy()	{
			return new SnailLiteral(value);
		}
	}
	...
	public static void main(String[] args) throws IOException	{
		...
		long max=0;
		for (int i=0;i<numbers.size();++i) for (int j=0;j<numbers.size();++j) if (i!=j)	{
			SnailNumber left=numbers.get(i).copy();
			SnailNumber right=numbers.get(j).copy();
			SnailNumber number=left.add(right);
			number.normalise();
			long magnitude=number.getMagnitude();
