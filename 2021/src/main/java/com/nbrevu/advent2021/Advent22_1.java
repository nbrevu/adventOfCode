...
public class Advent22_1 {
	...
	private static boolean isWithinBounds(Cuboid c)	{
		return (Math.abs(c.x0)<=50)&&(Math.abs(c.x1)<=50)&&(Math.abs(c.y0)<=50)&&(Math.abs(c.y1)<=50)&&(Math.abs(c.z0)<=50)&&(Math.abs(c.z1)<=50);
	}
