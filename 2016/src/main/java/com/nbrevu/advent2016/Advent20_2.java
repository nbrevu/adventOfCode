...
public class Advent20_2 {
	...
	private final static long LIMIT=4294967296l;
	
	private static long countNotInRange(RangeSet<Long> ranges,long limit)	{
		long count=0l;
		long firstAvailable=0l;
		for (Range<Long> range:ranges.asRanges())	{
			long start=range.lowerEndpoint();
			long end=range.upperEndpoint();
			if (start>firstAvailable) count+=start-firstAvailable;
			firstAvailable=1+end;
		}
		if (firstAvailable<limit) count+=limit-firstAvailable;
		return count;
	}
	...
	public static void main(String[] args) throws IOException	{
		...
		System.out.println(countNotInRange(ranges,LIMIT));
	}
}
