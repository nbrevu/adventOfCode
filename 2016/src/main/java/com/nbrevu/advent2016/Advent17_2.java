...
public class Advent17_2 {
	...
	private static int findLongestPath(FullPosition initial,HashCalculator calculator)	{
		return findLongestPathRecursive(initial,calculator,1);
	}
	
	private static int findLongestPathRecursive(FullPosition position,HashCalculator calculator,int currentDepth)	{
		int maxFound=-1;
		for (FullPosition p:position.getChildren(calculator))	{
			if (p.isFinal()) maxFound=Math.max(maxFound,currentDepth);
			else	{
				int result=findLongestPathRecursive(p,calculator,1+currentDepth);
				if (result>=0) maxFound=Math.max(result,maxFound);
			}
		}
		return maxFound;
	}
	...
	public static void main(String[] args) throws NoSuchAlgorithmException	{
		...
		int result=findLongestPath(initial,calculator);
