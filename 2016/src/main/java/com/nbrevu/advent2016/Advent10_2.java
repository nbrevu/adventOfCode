...
public class Advent10_2 {
	...
	private final static int[] GOAL=new int[] {0,1,2};
	...
	private static class Bot	{
		...
		public Bot(String lowOutputType,int lowOutputId,String highOutputType,int highOutputId)	{
		...
		public IntIntMap assignments(String outputType)	{
			IntIntMap result=HashIntIntMaps.newMutableMap();
			int[] values=values();
			if (lowOutputType.equals(outputType)) result.put(lowOutputId,values[0]);
			if (highOutputType.equals(outputType)) result.put(highOutputId,values[1]);
			return result;
		}
	}
	
	private static void assign(Bot bot,int value,IntObjMap<Bot> bots,IntIntMap outputs)	{
		bot.receive(value);
		if (bot.isComplete())	{
			IntIntMap pass=bot.assignments("bot");
			for (IntIntCursor cursor=pass.cursor();cursor.moveNext();) assign(bots.get(cursor.key()),cursor.value(),bots,outputs);
			IntIntMap toWrite=bot.assignments("output");
			for (IntIntCursor cursor=toWrite.cursor();cursor.moveNext();)	{
				if (outputs.containsKey(cursor.key())) throw new IllegalArgumentException("You'll need to use an eraser before that...");
				outputs.put(cursor.key(),cursor.value());
			}
		}
	}
	
	private static void assign(IntIntMap values,IntObjMap<Bot> bots,IntIntMap outputs)	{
		for (IntIntCursor cursor=values.cursor();cursor.moveNext();) assign(bots.get(cursor.value()),cursor.key(),bots,outputs);
	}
	
	private static long calculateOutput(IntIntMap outputs,int[] indices)	{
		long result=1;
		for (int i:indices) result*=outputs.get(i);
		return result;
	}
	...
	public static void main(String[] args) throws IOException	{
		...
		IntIntMap outputs=HashIntIntMaps.newMutableMap();
		assign(values,bots,outputs);
		System.out.println(calculateOutput(outputs,GOAL));
	}
}
