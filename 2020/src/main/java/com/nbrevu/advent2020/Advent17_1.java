...
public class Advent17_1 {
	...
	private static int countNeighbours(boolean[][][] array,int i,int j,int k)	{
		int result=0;
		for (int x=i-1;x<=i+1;++x) for (int y=j-1;y<=j+1;++y) for (int z=k-1;z<=k+1;++z) if ((x==i)&&(y==j)&&(z==k)) continue;
		else if (isActive(array,x,y,z)) ++result;
		return result;
	}
	...
	private static boolean[][][] iterate(boolean[][][] in)	{
		...
		boolean[][][] result=new boolean[expanded1][expanded2][expanded3];
		for (int i=0;i<expanded1;++i) for (int j=0;j<expanded2;++j) for (int k=0;k<expanded3;++k)	{
			boolean previousValue=isActive(in,i-1,j-1,k-1);
			int neighbours=countNeighbours(in,i-1,j-1,k-1);
			if ((neighbours==3)||((neighbours==2)&&previousValue)) result[i][j][k]=true;
		}
		return result;
	}
	...
	private static int countActive(boolean[][][] array)	{
		int result=0;
		for (int i=0;i<array.length;++i) for (int j=0;j<array[i].length;++j) for (int k=0;k<array[i][j].length;++k) if (array[i][j][k]) ++result;
		return result;
	}
	
	public static void main(String[] args) throws IOException	{
		URL file=Resources.getResource(IN_FILE);
		boolean[][][] configuration=new boolean[1][][];
		configuration[0]=Resources.readLines(file,Charsets.UTF_8).stream().map(Advent17_1::parse).toArray(boolean[][]::new);
