...
public class Advent15_12_2 {
	private final static String IN_FILE="Advent12.txt";
	
	private static boolean isRed(JsonElement element)	{
		if (!element.isJsonPrimitive()) return false;
		JsonPrimitive value=element.getAsJsonPrimitive();
		if (!value.isString()) return false;
		return "red".equals(value.getAsString());
	}
	...
	private static long addNumbers(JsonElement element)	{
		if (element.isJsonPrimitive())	{
			...
			long result=0;
			for (Map.Entry<String,JsonElement> entry:object.entrySet())	if (isRed(entry.getValue())) return 0l;
			else result+=addNumbers(entry.getValue());
			return result;
